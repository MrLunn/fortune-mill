...
async function handleApi(req, res) {
  ...
  if (url === '/api/register' && method === 'POST') {
    ...
    const p = { username, token: token(), state: null, netWorth: 0, lifetime: 0, combat: { atk: 1, luck: 0, hp: 100 }, guild: null, wins: 0, losses: 0, created: Date.now() };
    db.players[username.toLowerCase()] = p;
    saveDB();
    return sendJSON(res, 200, { username: p.username, token: p.token, state: null });
  }
  ...
  if (url === '/api/login' && method === 'POST') {
    ...
    return sendJSON(res, 200, { username: p.username, token: p.token, state: p.state, guild: p.guild });
  }
