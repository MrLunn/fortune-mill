 ...
 * Rooms: Darts, Scratchers, Slots. Plus PvP, Guilds, Prestige, Challenges.
 */
(() => {
  'use strict';
  ...
  // Prestige-2 unlocked dart upgrades
  const DART_ELITE = [
  ...
  // ── Daily challenges ───────────────────────────────────────────────────────
  const CH_TYPES = [
    { id:'throw_count',  icon:'🎯', name:'Dart Marathon',    desc:n=>`Throw ${n} darts`,           targets:[50,150,400,1000], reward:n=>n*12,     xp:n=>n     },
    { id:'crits',        icon:'💥', name:'Critical Striker', desc:n=>`Land ${n} critical throws`,  targets:[5,20,40,75],     reward:n=>n*200,    xp:n=>n*3   },
    { id:'todayEarned',  icon:'💰', name:'Gold Rush',        desc:n=>`Earn ${money(n)} today`,     targets:[5e3,5e4,5e5,5e6],reward:n=>n*0.15,   xp:_=>50    },
    { id:'scratchWins',  icon:'🎟', name:'Lucky Scratcher',  desc:n=>`Win ${n} scratch tickets`,   targets:[5,15,35,70],     reward:n=>n*250,    xp:n=>n*4   },
    { id:'jackpots',     icon:'💎', name:'Jackpot Hunter',   desc:n=>`Hit ${n} jackpots`,          targets:[1,3,7,15],       reward:n=>n*12000,  xp:n=>n*25  },
    { id:'slotWins',     icon:'🎰', name:'Slot Star',        desc:n=>`Win ${n} slot spins`,        targets:[5,20,50,100],    reward:n=>n*400,    xp:n=>n*6   },
  ];
  ...
  function getGuildBuff() {
    // buff unlocked at treasure thresholds
  ...
  function perSecond() {
    ...
    return Math.max(0, fromDarts + Math.max(0,ev) + Math.max(0,slotEv));
  }
  ...
  function earn(amount) {
    ...
    // Today earnings (for challenge tracking)
  ...
  function checkClear(room, goalId, textId) {
    ...
    if (!state[room]?.cleared && best >= ROOM_GOAL) {
      ...
      toast(`🎉 ${room} room CLEARED! Synergy bonus unlocked.`);
      updatePrestigeBtn();
  ...
  // ── Focus meter ────────────────────────────────────────────────────────────
  function addFocus(amt) {
    state.focus = Math.min(100, (state.focus || 0) + amt);
    _renderFocus();
  }
  ...
  function throwDart() {
    ...
    Anim.floatFromEl('+'+money(payout), $('throwBtn'), crit?'#ffd166':'#00e87a');
    Music.sfxDart(crit);
    refreshTop();
  ...
  function scratchTicket() {
    ...
    if (won) {
      ...
      Anim.floatFromEl('+'+money(win), $('scratchBtn'), jackpot?'#ffd166':'#00e87a');
  ...
  function spinSlots() {
    ...
    // Pick results — luck biases subsequent reels toward reel 0
  ...
  function _resolveSlots(bet, res) {
    ...
    if (three) {
      ...
      logCls = jackpot ? 'good' : '';
      Anim.floatFromEl('+'+money(win), $('slotBtn'), jackpot?'#ffc820':'#00e87a');
      if (jackpot) { Music.sfxWin(true); if(typeof Dopamine!=='undefined') Dopamine.shake('heavy'); }
      else         { Music.sfxWin(false);if(typeof Dopamine!=='undefined') Dopamine.shake('light'); }
    } else if (two) {
      ...
      Anim.floatFromEl('+'+money(win), $('slotBtn'), '#33aaff');
    } else {
  ...
  function _seedRng(seed) {
    return (n) => {
      let x = Math.sin(seed * 9301 + n * 49297 + 233) * 1e5;
  ...
  function updateChallengeProgress(fieldId, delta) {
    ...
    for (const ch of state.challenges) {
      ...
      // Accumulating counter-based challenges
  ...
  function _renderPrestigeLocked(containerId, eliteList) {
    ...
    if (auth.prestige < 2) {
      ...
      eliteList.forEach(def => {
        const level = lvl(def.id.includes('dart')||def.id.includes('elite_power')||def.id.includes('elite_combo') ? 'darts' : 'scratch', def.id);
  ...
  function renderAffordability() {
    qsa('#dartsUpgrades .upgrade, #scratchUpgrades .upgrade, #slotsUpgrades .upgrade').forEach(row => {
  ...
  function refreshTop() {
    ...
    $('dartCrit').textContent  = (dartCrit()*100).toFixed(0) + '%';
    $('scratchLuck').textContent = (25 + scratchLuck()*100).toFixed(0) + '%';
    $('scratchAuto').textContent = scratchAuto() + '/s';
    $('myAtk').textContent    = c.atk;
    $('myLuck').textContent   = c.luck;
    $('myHp').textContent     = c.hp;
    $('slotAutoDisplay').textContent = slotAuto() > 0 ? slotAuto() + '/s' : 'Off';
    renderAffordability();
    _renderFocus();
  }
  ...
  function tick() {
    ...
    // Focus decay
    state.focus = Math.max(0, (state.focus||0) - 0.25*dt*20);  // ~0.25/s
  ...
  function updatePrestigeBtn() {
    ...
    btn.title    = canPrestige() ? 'Reset progress for +50% permanent earnings' : 'Clear both rooms first';
  }
  ...
  async function doPrestige() {
    ...
    try {
      ...
      state.slots   = {cleared:false,best:0,upgrades:{}};
  ...
  function renderGuildBuff() {
    ...
    if (buff) {
      el.innerHTML = `<div class="guildbuff">🛡 ${buff.label}</div>`;
    } else {
      el.innerHTML = '';
    }
  ...
  function loadState(blob) {
    ...
    try {
      ...
      state.slots   = Object.assign({cleared:false,best:0,upgrades:{}}, s.slots);
  ...
  function startGame() {
    ...
    if (typeof Dopamine !== 'undefined') {
      ...
      if (offline > 0) {
        earn(offline); refreshTop();
        toast(`💤 Offline: +${money(offline)} earned while away (50% rate, 8h cap)`);
      }
    }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('keydown', e=>{
      if (e.code==='Space' && !$('app').classList.contains('hidden')) {
        const active = !qs('section[data-view="darts"]').classList.contains('hidden');
        const typing = ['INPUT','SELECT','TEXTAREA'].includes(document.activeElement.tagName);
        if (active && !typing) { e.preventDefault(); throwDart(); }
      }
    });
  });
})();
